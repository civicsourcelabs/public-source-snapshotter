...
MHLW monthly collectorは、private monthly processing scheduleより前にencrypted full artifactを生成します。
...
Navii detail collectorは、全件取得負荷と利用判断が残るため、このrunbook時点ではmanual dispatchのまま扱います。
