...
| `Source Snapshot: Navii Detail` | 6月/12月の4,7,10,13,16,19,22,25,28日 00:30 JST / 1月/7月の3,10,17,24,31日 00:30 JST | `execute=true`、`artifact_mode=encrypted_full`、`candidate_mode=all`、`shard_count=16`、`max_parallel=16`、`max_pages_per_shard=0`、`artifact_retention_days=14` |
...
Navii detail runでは、Asia/Tokyoの実行日から半期の期待snapshot dateを自動生成します。6月から11月は同年 `YYYY-06-01`、12月から翌5月は直近の `YYYY-12-01` を期待し、公式open data pageに該当snapshotと必須linkがなければfail closedします。run labelも `collector-navii-detail-YYYYMMDD-{canary|scope|full}` として自動生成します。scheduleはreadiness pollとして複数回動きますが、同じsnapshot dateのfull artifactが既に成功していれば後続jobをskipします。
