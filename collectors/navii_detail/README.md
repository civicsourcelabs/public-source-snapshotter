...
non-zero section/table/link/phone extraction for every requested kind. The
full workflow's preflight uses the same extraction checks across its
approximately 0.1% sample and records the result in the preflight audit
artifact; its sampled rows are not used as the full handoff input.
